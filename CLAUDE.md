...
- **Always merge stacks to `main`.** Never merge a PR into another feature branch expecting it to cascade — that orphaned the Phase 04 stack once. The `rebase-descendants.yml` CI workflow rebases stacked children automatically when their base PR merges.
